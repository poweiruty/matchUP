...
public class CorpUserDao {
	...
	public CorpUser getUserbyCname(String cname) {
		CorpUser cuser = null;
		this.conn = DBManager.getConnection();

		if (this.conn != null) {
			String sql = "SELECT * FROM cusers_tb WHERE cname=?";
			try {
				this.pstmt = this.conn.prepareStatement(sql);
				...
				if (this.rs.next()) {
					...
					cuser = new CorpUser(id, password, name, num, mgr_name, mgr_tel, mgr_email, address);				
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
